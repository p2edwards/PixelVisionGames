...
LoadScript("code-micro-platformer")
LoadScript("code-entities")
...
function GameScene:Reset()
  ...
  -- Draw UI
  DrawSprites(borderleft.spriteIDs, 21 * 8, 0, borderleft.width, false, false, DrawMode.TilemapCache, 0, false, false)
  DrawSprites(bordertop.spriteIDs, 23 * 8, 0, bordertop.width, false, false, DrawMode.TilemapCache, 0, false, false )
  DrawSprites(borderleft.spriteIDs, (17 + 21) * 8, 0, borderleft.width, true, false, DrawMode.TilemapCache, 0, false, false )

  -- Create two clouds
  for i = 1, 2 do

    local cloud = Cloud:Init(
      math.random(0, levelSize.x) * 8,
      math.random(7, levelSize.y - 6) * 8,
      i < 2 and "cloudsmall" or "cloudlarge",
      math.random(5, 10)
    )
  ...
  for i = 1, self.totalLevelTiles do
    ...
    if(flag == EXIT) then
      ...
      DrawSprites(doorlocked.spriteIDs, self.exitPos.c, self.exitPos.r, doorlocked.width, false, false, DrawMode.Tile)
  ...
  for i = 1, totalStars do
    ...
    local tmpC = 2 + ((i - 1) * staroff.width)
    local tmpR = 0

    DrawSprites(staroff.spriteIDs, (tmpC + 21) * 8, 3, staroff.width, false, false, DrawMode.TilemapCache, 0, false, false)
...
function GameScene:Update(timeDelta)
  ...
  if(self.playerEntity.alive == false) then
    ...
    if(wasAlive == true) then
      ...
      local tombPixelData = Sprite(tomb.spriteIDs[1])
  ...
  if(stars == totalStars and self.unlockDoor == false) then
    ...
    -- TODO this is not replacing the tiles correctly after changing the UpdateTiles API
    DrawSprites(dooropen.spriteIDs, self.exitPos.c, self.exitPos.r, dooropen.width, false, false, DrawMode.Tile)
  ...
  if(totalCollected > 0) then
    ...
    for i = 1, totalCollected do
      ...
      local tmpC = 2 + ((id - 1) * staron.width)
      local tmpR = 1

      DrawSprites(staron.spriteIDs, (tmpC + 21) * 8, 3, staron.width, false, false, DrawMode.TilemapCache, 0, false, false)
