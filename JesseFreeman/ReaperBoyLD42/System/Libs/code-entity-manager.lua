...
function UpdateEntities(timeDelta, scrollPos)
  ...
  for i = 1, #entities do
    ...
    if(entity.remove ~= true) then
      ...
      -- if(bounds.IntersectsWith(tmpRect)) then
    ...
    if(entity.visible == false) then
      ...
      if(entity.remove ~= true) then
        ...
        if(entity.spriteData ~= nil) then
          DrawSprites(entity.spriteData.spriteIDs, entity.pos.x, entity.pos.y, entity.spriteData.width, entity.flipH, false, entity.drawMode, entity.colorOffset, entity.onScreen, entity.useScrollPos)
        end
